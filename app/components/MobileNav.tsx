...
export default function MobileNav({
  ...
  return (
    <motion.nav
      ...
      className="fixed top-0 left-0 h-screen w-full bg-cover bg-black text-white pt-4 pb-24 px-6 flex flex-col gap-16 lg:hidden z-40"
        ...
        {menuItems.map(({ key, href }) => (
          <a
            ...
            className={`text-sm font-medium ${
              activeItem === href ? 'text-primary-light' : 'text-white'
            }`}
            onClick={() => {
              setActiveItem(href);
              setMobileOpen(false);
